...
def inventory():
    ...
    rows=''.join([f'<tr><td>{i["sku"]}</td><td>{i["name"]}</td><td>{i["department"]}</td><td>KES {i["unit_price"]:,.2f}</td><td>{i["stock_qty"]}</td><td>{i["reorder_level"]}</td></tr>' for i in items])
    return page('Department Inventory','Administrator view of stock levels across all sections',f'<div class="card"><table><tr><th>SKU</th><th>Item</th><th>Department</th><th>Price</th><th>Stock</th><th>Reorder</th></tr>{rows}</table></div>')
